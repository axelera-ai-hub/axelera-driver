...
static int edma_dma_irq_ck(struct axl_pcie_aipu_dev *axldev, int id)
{
	struct dw_edma_v0_regs *edma = axldev->dma;
	struct pci_dev *pdev = axldev->pdev;
	__u32 ctrl;
	int ch = id <= MSI_RD_CH3 ? id - MSI_RD_CH0 : id - MSI_WR_CH0;
	...
	if (DW_EDMA_CTRL_CS(ctrl) == DW_EDMA_V0_CS_STOP) {
		dev_dbg(&pdev->dev, "wake (%d) DMA %s CH%d (ctrl 0x%x)\n", id,
			id <= MSI_RD_CH3 ? "RD" : "WR", ch, ctrl);
		return 1;
	}
	return 0;
}

static void edma_enable_ctrl(struct axl_pcie_aipu_dev *axldev)
{
	struct dw_edma_v0_regs *edma = axldev->dma;
...
static void edma_init_imwr(struct axl_pcie_aipu_dev *axldev)
{
	...
	edma->rd_ch01_imwr_data = MSI_RD_CH0 | (MSI_RD_CH1 << 16);
	edma->rd_ch23_imwr_data = MSI_RD_CH2 | (MSI_RD_CH3 << 16);
	edma->wr_ch01_imwr_data = MSI_WR_CH0 | (MSI_WR_CH1 << 16);
	edma->wr_ch23_imwr_data = MSI_WR_CH2 | (MSI_WR_CH3 << 16);
}

static void edma_align_imwr(struct axl_pcie_aipu_dev *axldev)
{
	volatile struct dw_edma_v0_regs *edma = axldev->dma;
	u32 imwr_data;
			       ...
			       struct dma_wrk *dma_wrk)
{
	...
	ctrl = GET_RW_32_CH(edma, dir, ch_control1, dma_wrk->channel);
	if (err == 0) {
		dev_err(&pdev->dev, "DMA %s CH%d timeout (irq %d, ctrl 0x%x)\n",
			mode, dma_wrk->channel, dma_wrk->id,
	...
	if (DW_EDMA_CTRL_CS(ctrl) != DW_EDMA_V0_CS_STOP) {
		dev_err(&pdev->dev, "DMA error %s CH%d (ctrl 0x%x)\n", mode,
			dma_wrk->channel, DW_EDMA_CTRL_CS(ctrl));
		dma_wrk->status = -EIO;
...
static void edma_dma_job(struct dma_wrk *dma_wrk)
{
	...
	hwlldch = (struct dw_edma_ll_buf *)(EDMA_L2_LINKED_LIST_DESC_BASE);
	lldch = axldev->vl2base + EDMA_L2_LINKED_LIST_DESC_OFF;
	dma_wrk->status = 0;
...
static void edma_dma_p2p_job(struct dma_wrk *dma_wrk)
{
	struct axl_pcie_aipu_dev *axldev = dma_wrk->axldev;
	struct pci_dev *pdev = axldev->pdev;
...
static struct axl_dev_fops edma_fops = {
	.dma_irq_ck = edma_dma_irq_ck,
	.dma_enable_ctrl = edma_enable_ctrl,
	.dma_job_submit = edma_dma_job,
	.dma_p2p_job_submit = edma_dma_p2p_job,
	.dma_init_imwr = edma_init_imwr,

	.dev_debugfs_init = edma_dev_debugfs_init,
	.dev_debugfs_exit = edma_dev_debugfs_exit,
};

void edma_register_dev_fops(struct axl_pcie_aipu_dev *axldev)
{
	axldev->fops = &edma_fops;
}
